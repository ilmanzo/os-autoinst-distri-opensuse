...
sub run {
    ...
    script_run('timedatectl status'); # show clock
    ...
    for my $sec ('sys', 'krb5', 'krb5i', 'krb5p') {
        ...
        my $ret=script_run "mount -t nfs4 -o rw,sec=$sec $dom_server:$nfs_expdir $nfs_mntdir";
        if ($ret and is_sle('=15-SP5')) {
            record_soft_failure('bsc#1223688');
            next;
        }
