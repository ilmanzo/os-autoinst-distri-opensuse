...
sub check_sealert() {
    ...
    my $sample_file = '/tmp/sealert_sample_test';
    ...
    die "Should deny permission" unless script_run("runcon -u guest_u -r guest_r -t user_tmp_t -- cat $sample_file");
    # sealert should show deny message
    my $sealert_output;
    my $retries = 10;
    while ($retries--) {
        my $glob_output = script_output "sealert -l '*'";
        record_info 'sealert output', $glob_output;
        last if valid_sealert_output($glob_output);
        sleep 1;
    }
    die "sealert -l '*' Does not validate" unless $retries;
    # extract event id from the output
    my $local_id;
    if ($sealert_output =~ /Local ID\s+([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})/s) {
        $local_id = $1;
    } else {
        die "alert event ID not found";
    }
    # run same validations against specific ID output
    die "sealert -l $local_id does not validate" unless valid_sealert_output(script_output "sealert -l $local_id");
    # we should find deny message in journal as well
    assert_script_run "journalctl -u setroubleshootd.service | grep 'SELinux is preventing runcon from using the transition access'";
}
