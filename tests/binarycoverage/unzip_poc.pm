...
sub run {
    ...
    my $cov = binarycoverage::functions_coverage->new('/usr/bin/unzip', 'unzip');
    ...
    $cov->call(\&assert_script_run, 'unzip test_unzip.zip');
    $cov->report(1);
    ...
    # 2. Unzip into a new directory
    $cov->call(\&assert_script_run, 'unzip -d extract/ test_unzip.zip');
    ...
    # 3. Test the credibility of the zip archive
    $cov->call(\&assert_script_run, 'unzip -tq test_unzip.zip');
    $cov->report(1); 
